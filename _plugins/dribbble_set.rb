...
module Jekyll
  class DribbbleSetTag < Liquid::Tag
    def initialize(tag_name, config, token)
      ...
      userMatch = /(?<=user:)\w+/
      shotsPerPageMatch = /(?<=shots_per_page:)\w+/
      pageMatch = /(?!page:)\d+(?=\sshots_per_page)/
      ...
      if (config =~ shotsPerPageMatch) 
        @shots_per_page          = shotsPerPageMatch.match(config)[0]
      end
      @shots_per_page            ||= @config['shots_per_page']
      @shots_per_page            ||= 15 # max 30

      if (config =~ pageMatch) 
        @page                    = pageMatch.match(config)[0]
      end
      @page                      ||= @config['page']
      @page                      ||= 1
